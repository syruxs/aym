...
<header>
  <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
      ...
      <div class="collapse navbar-collapse" id="navbarCollapse">
		...
		if($perfil == "ADMINISTRADOR"){
			echo '
				...
				<li class="nav-item">
					<a class="nav-link" aria-current="page" href="flota/flota.php">FLOTA</a>
				</li>
			';
		}if($perfil == "DIVISA"){
			echo '
	...
	<select class="form-control" name="abonado" id="abonado">
	...
	<select class="form-control" name="usuario" id="usuario">
		<option value="TODOS">TODOS</option>
		<?php 
			$Login=mysqli_query($conn, "SELECT * FROM `login` ORDER BY name ASC");
				while($Rl=mysqli_fetch_array($Login)){
					echo '
						<option value="'.$Rl['user'].'">'.$Rl['name'].'</option>
					';
				}
		?>
	</select>&nbsp;&nbsp;
    ...
    <div class="table-responsive">
            <table class="table table-striped table-hover" style="background-color: white; color: rgb(110, 108, 108); font-size: 12px;">
                <tr>
                    <th>N°</th>
                    <th>Tracto</th>
                    <th>Semi</th>
				...
				if($filter=="ACTIVO" && $Abono=="TODOS" && $L=="TODOS"){
				...
				}if($filter=="ACTIVO" && $Abono=="SIN ABONO" && $L=="TODOS"){
			        $DATE_X="fecha_ingreso";

                    $sql = mysqli_query($conn, "SELECT * FROM `ingreso` where estado='$filter' and $DATE_X BETWEEN '$date1' AND '$date2' AND abono='' ORDER BY id ASC");				
				}if($filter=="ACTIVO" && $Abono=="CON ABONO" && $L=="TODOS"){
			        $DATE_X="fecha_ingreso";

                    $sql = mysqli_query($conn, "SELECT * FROM `ingreso` where estado='$filter' and $DATE_X BETWEEN '$date1' AND '$date2' AND abono!='' ORDER BY id ASC");
				}if($filter=="INACTIVO" && $L=="TODOS"){
                    ...
                    $sql = mysqli_query($conn, "SELECT * FROM `ingreso` where usuario_salida='$L' AND estado='$filter' AND $DATE_X BETWEEN '$date1' AND '$date2' ORDER BY id ASC");
					
					$Sql = mysqli_query($conn, "SELECT SUM(pago) as pago FROM `ingreso` WHERE usuario_salida='$L' AND estado='INACTIVO' AND fecha_salida BETWEEN '$date1' AND '$date2' ORDER BY id ASC ");	
					
					$SQL = mysqli_query($conn, "SELECT SUM(cancelado) as cancelado FROM `ingreso` WHERE usuario_salida='$L' AND estado='INACTIVO' AND fecha_salida BETWEEN '$date1' AND '$date2' ORDER BY id ASC ");				
				}
